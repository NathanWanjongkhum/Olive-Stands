...
const addGame = async (req, res) => {
    ...
    try {
        ...
        let matchingGenres = await client.query('SELECT "ID" FROM genre WHERE "Genre"=$1', [genre]);
        if (matchingGenres.rowCount === 0) {
            matchingGenres = await client.query('INSERT INTO genre ("Genre") VALUES ($1) RETURNING *', [genre]);
        }
