...
CREATE TYPE GENRE_NAME AS ENUM('Action','Adventure','RPG','Strategy','Simulation','Sports','Puzzle','Shooter','Platformer',
...
CREATE TABLE Users (
    ...
    CONSTRAINT valid_pw_length CHECK(LENGTH("Password") >= 8)
);
...
CREATE TABLE Game (
    "ID" SERIAL PRIMARY KEY,
    "Name" TEXT NOT NULL,
    "Genre" SERIAL REFERENCES Genre("ID") ON DELETE CASCADE ON UPDATE CASCADE,
    "Preview" TEXT DEFAULT NULL,  -- IMAGE type replaced with BLOB
    "Description" VARCHAR(1000) DEFAULT 'No description provided',
    "Link" TEXT,
    CONSTRAINT valid_game_name CHECK (TRIM("Name") != ''),
    CONSTRAINT valid_link CHECK ("Link" LIKE 'https://%.github.io%')
);

CREATE TABLE Feedback (
    "User_ID" SERIAL REFERENCES Users("ID") ON DELETE CASCADE ON UPDATE CASCADE,
    "Game_ID" SERIAL REFERENCES Game("ID") ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE Developers (
    ...
    "Game_ID" INT REFERENCES Game("ID") ON DELETE CASCADE ON UPDATE CASCADE
);
...
INSERT INTO Game ("Name", "Genre", "Preview", "Description", "Link") VALUES 
    ...
    ('Mega Maze Mayhem', 19, NULL, 'Join Mega Man as he navigates through a labyrinth filled with dangerous enemies to find the exit', 'https://baott05.github.io/webGame/'),
...
Purpose: Selects all reviews from games made by Josh Kang
...
WHERE "D"."Name" LIKE '%Josh Kang%';

/* 
Query 10: A non-trivial query using at least three tables with aliasing/renaming
Purpose: Selects all usernames that rated the game 'Grandmas vs. Unhappiness' 5 stars
...
FROM Users AS "U"
    ...
    JOIN Game AS "G" ON "G"."ID" = "F"."Game_ID"
WHERE "F"."Rating" = 5 AND "G"."Name" = 'Grandmas vs. Unhappiness'